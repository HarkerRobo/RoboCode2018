...
import org.usfirst.frc.team1072.robot.profiling.LocalMotionBuilder.Group;
 ...
 */
public class LocalMotionCommand extends Command {
	 ...
	 * builder instead)
	 */
	public LocalMotionCommand(int period, Group[] groups, Subsystem[] required) {
		this.period = period;
		this.groups = groups;
	...
	protected boolean isFinished() {
		...
		for(int i = 0; i < statuses.length; i++)
			for(int j = 0; j < statuses[i].loadNext.length; j++)
				if(statuses[i].loadNext[j] != groups[i].getPath().getPath().length)
					return false;
		for(Group g : groups)
			for(TalonSRX t : g.getTargets()) {
...
//					tp.auxiliaryPos = trajectory.segments[statuses[i].loadNext[j]].heading * 180.0 * 16.0 / Math.PI;
					tp.velocity = trajectory[statuses[i].loadNext[j]][0]
...
//					tp.headingDeg = trajectory.segments[statuses[i].loadNext[j]].heading * 180.0 * 64.0 / Math.PI;
					...
					tp.isLastPoint = ++statuses[i].loadNext[j] == trajectory.length;
